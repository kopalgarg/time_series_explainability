...
def evaluate_multiclass(labels, predicted_label, predicted_probability, task='multiclass'):
    ...
    if task=='multiclass':
        if len(np.unique(np.argmax(labels_array,1))) >= 2: # multiple classes 
        ...
        for i, (signals, labels) in enumerate(train_loader):
            ...
            for t in time_points:
                ...
                if type(loss_criterion).__name__==type(torch.nn.CrossEntropyLoss()).__name__:
                    ...
                    label_onehot = torch.zeros(label.shape).to(device)
...
def train_model_rt_mod(model, train_loader, valid_loader, optimizer, n_epochs, device, experiment, data='simulation',
                   num=5,cv=0):
    ...
    for epoch in range(n_epochs):
        ...
        for i, (signals, labels) in enumerate(train_loader):
            ...
            for t in time_points:
                ...
                reconstruction_loss = loss_criterion(predictions, np.argmax(label_onehot,axis=1))
                epoch_loss += reconstruction_loss.item()
                reconstruction_loss.backward()
                optimizer.step()
...
def test_model_rt_mod(model, test_loader, num=1):
    ...
    for i, (signals, labels) in enumerate(test_loader):
        ...
        for t in [int(tt) for tt in np.linspace(0, signals.shape[2] - 2, num=num)]:
            label = labels[:,:,t]
            label_onehot = torch.FloatTensor(label.shape).to(device)
            pred_onehot = torch.FloatTensor(label.shape).to(device)
            preds = model(signals[:, :, :t + 1])
            ...
            count += 1
            loss_criterion = torch.nn.CrossEntropyLoss()
            loss = loss_criterion(predictions, np.argmax(label_onehot,axis=1).to(device))
            test_loss += loss.item()
